...
import com.bytatech.ayoos.doctor.config.KafkaProperties;
...
class DoctorKafkaResourceIT {
    ...
    void setup() {
        ...
        DoctorKafkaResource kafkaResource = new DoctorKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/doctor-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/doctor-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
